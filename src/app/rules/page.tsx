  ...
  return (
    <div className="min-h-screen bg-background text-white">
      ...
      <div className="max-w-3xl mx-auto px-4 pb-16 space-y-8">
        {/* ── Starting balance ─────────────────────────────────────────── */}
        <section className="rounded-2xl border border-accent/30 bg-gradient-to-br from-accent/10 to-surface p-6">
          ...
          <p className="text-zinc-300 text-sm leading-relaxed">
            ...
            <span className="font-black text-accent">{STARTING_BALANCE} pts</span>{' '}
            when they join. Each prediction costs a{' '}
            <span className="font-black text-accent">{PREDICTION_STAKE} pt stake</span>{' '}
          ...
          <div className="mt-4 grid grid-cols-4 gap-3 text-center">
            <div className="rounded-xl bg-surface border border-rim p-3">
              <p className="text-2xl font-black text-accent">{STARTING_BALANCE}</p>
            ...
            <div className="rounded-xl bg-surface border border-rim p-3">
              <p className="text-2xl font-black text-positive">+30</p>
        ...
        <section>
          ...
          <div className="rounded-2xl border border-rim overflow-hidden">
            <div className="grid grid-cols-[1fr_auto] gap-x-4 px-4 py-2.5 bg-surface border-b border-rim text-[10px] font-semibold text-zinc-500 uppercase tracking-wider">
              <span>Condition</span>
          ...
          <p className="mt-2 text-[11px] text-zinc-500 text-center">
            ...
            With early-prediction bonus applied: up to <strong className="text-positive">+30 pts</strong>.
          </p>
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {SCORE_TOLERANCES.map((t) => (
              <div
                key={t.format}
                className="rounded-xl bg-surface border border-rim p-4 text-center"
              >
                <p className="text-sm font-black text-accent mb-1">{t.format}</p>
            ...
            {MULTIPLIER_RULES.map((r, i) => (
              <div
                ...
                className={`flex items-center justify-between px-4 py-3.5 ${
                  i < MULTIPLIER_RULES.length - 1 ? 'border-b border-rim' : ''
                } ${i === 0 ? 'bg-positive/5' : ''}`}
              >
                <span className="text-sm text-white">{r.window}</span>
                <span
                  className={`text-sm font-black ${
                    r.multiplier.startsWith('×1.0') ? 'text-zinc-500' : 'text-positive'
                  }`}
                >
                  {r.multiplier}
                </span>
        ...
        {/* ── Tips ─────────────────────────────────────────────────────── */}
        <section className="rounded-2xl border border-rim bg-surface p-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center pt-2">
          <a
            href="/"
            className="px-6 py-3 bg-accent text-white font-black rounded-xl hover:opacity-90 transition-opacity text-sm text-center"
          >
            Make Predictions
          </a>
          <a
            href="/leaderboard"
            className="px-6 py-3 border border-rim text-zinc-300 font-bold rounded-xl hover:bg-rim transition-colors text-sm text-center"
          >
            View Leaderboard
          </a>
        </div>
