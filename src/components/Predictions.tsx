...
function PredictionCard({ match, prediction, onPredict, onUpdate }: PredictionCardProps) {
  ...
  const handleSubmit = () => {
    if (!winner) return;
    if (prediction) {
      onUpdate(winner, homeRuns, awayRuns);
    } else {
      onPredict(winner, homeRuns, awayRuns);
    }
    setEditing(false);
  };
...
export function PredictionsPanel({ matches, userId = 'anonymous' }: PredictionsPanelProps) {
  ...
  const handleUpdate = (matchId: string) => (winner: string, homeRuns: number, awayRuns: number) => {
    ...
    if (existing) {
      setUserPredictions((prev) =>
        new Map(prev).set(matchId, { ...existing, predictedWinner: winner, predictedHomeRuns: homeRuns, predictedAwayRuns: awayRuns })
      );
    }
  };
